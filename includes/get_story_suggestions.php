...
try {
    ...
    $prompt = $promptManager->getPrompt('story', $type);

    // Build context string
    $context = "Genre: {$genre}";
    
    if (!empty($data['universe_title'])) {
        $context .= "\nPart of the universe: {$data['universe_title']}";
    }
    
    if (!empty($data['series_title'])) {
        $context .= "\nPart of the series: {$data['series_title']}";
    }

    // Format the prompt with variables
    $variables = [
        'count' => $count,
        'title' => isset($data['title']) ? strip_tags($data['title']) : 'the story',
        'genre' => $genre,
        'context' => $context
    ];
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_POSTFIELDS => json_encode([
            ...
            'messages' => [
                [
                    'role' => 'system',
                    'content' => 'You are a creative writing assistant helping to generate ideas for interactive stories.'
                ],
                [
                    'role' => 'user',
    ...
    error_log('Story Suggestions Error: ' . $e->getMessage());
