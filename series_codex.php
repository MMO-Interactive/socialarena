...
$stmt = $pdo->prepare("SELECT s.*, u.title as universe_title, u.id as universe_id FROM series s LEFT JOIN universes u ON s.universe_id = u.id WHERE s.id = ?");
...
$page_title = "Series Codex - " . htmlspecialchars($series['title']);
$additional_css = ['css/universe_codex.css'];

// Load entries: series-level + universe-level (if applicable)
$stmt = $pdo->prepare("
    SELECT ce.*, 
    ...
    WHERE (
        (ce.series_id = :seriesId)
        OR
        (ce.is_universe_level = TRUE AND ce.universe_id = :universeId)
    )
    ORDER BY ce.is_universe_level DESC, s.chronological_order ASC, ce.first_appearance_date ASC
");
$stmt->execute([
    'seriesId' => $series_id,
    'universeId' => $series['universe_id'] ?? 0
]);
...
if (!empty($entries)) {
    ...
    try {
        $stmt = $pdo->prepare("SELECT entry_id, related_entry_id FROM codex_relationships WHERE entry_id IN ($placeholders)");
...
if (!empty($entries)) {
    ...
    $placeholders = implode(',', array_fill(0, count($entry_ids), '?'));
    try {
        $stmt = $pdo->prepare("SELECT entry_id, tag_name FROM codex_tags WHERE entry_id IN ($placeholders) ORDER BY tag_name ASC");
        $stmt->execute($entry_ids);
    ...
    <div class="content-wrapper">
        ...
        <main class="main-content">
            <div class="codex-header">
                <h1><?php echo htmlspecialchars($series['title']); ?> - Series Codex</h1>
                <a href="create_codex_entry.php?series_id=<?php echo (int)$series_id; ?>" class="btn create-entry">
