...
$params = [$media_id];
$sql = "SELECT spm.*, s.title AS series_title, s.created_by, s.studio_id
...
if ($user_id) {
    $sql .= " OR s.created_by = ? OR (s.studio_id IN (SELECT studio_id FROM studio_permissions WHERE user_id = ? AND permission_key = 'series' AND allowed = 1))";
    $params[] = $user_id;
    $params[] = $user_id;
}
$sql .= ")";
...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM series_public_media_likes WHERE media_id = ?");
$stmt->execute([$media_id]);
$likeCount = (int)$stmt->fetchColumn();

$userLiked = false;
if ($user_id) {
    $stmt = $pdo->prepare("SELECT id FROM series_public_media_likes WHERE media_id = ? AND user_id = ?");
    $stmt->execute([$media_id, $user_id]);
    $userLiked = (bool)$stmt->fetchColumn();
}

$stmt = $pdo->prepare("SELECT c.*, u.username, u.profile_photo FROM series_public_media_comments c JOIN users u ON c.user_id = u.id WHERE c.media_id = ? ORDER BY c.created_at DESC");
$stmt->execute([$media_id]);
$comments = $stmt->fetchAll(PDO::FETCH_ASSOC);

$stmt = $pdo->prepare("
    SELECT id, title, release_title, media_type, thumbnail_url
    FROM series_public_media
    WHERE series_id = ? AND media_type = 'clip' AND id != ? AND release_status = 'released' AND visibility = 'public'
    ORDER BY released_at DESC, created_at DESC
    LIMIT 10
");
$stmt->execute([$media['series_id'], $media_id]);
                ...
                <div class="watch-grid">
                    <div class="watch-main">
                        ...
                        <div class="watch-title">
                            ...
                            <p><a href="series_public.php?id=<?php echo (int)$media['series_id']; ?>"><?php echo htmlspecialchars($media['series_title']); ?></a></p>
                        </div>
                        ...
                        <div class="watch-actions">
                            ...
                            <?php endif; ?>
                            <a class="btn btn-secondary" href="series_public.php?id=<?php echo (int)$media['series_id']; ?>">Back to Series</a>
                        </div>
                        ...
                        <div class="sidebar-card">
                            ...
                            <div class="suggested-list">
                                <?php foreach ($suggested as $s): ?>
                                    <a href="series_media_watch.php?id=<?php echo (int)$s['id']; ?>" class="suggested-item">
                                        <img src="<?php echo htmlspecialchars($s['thumbnail_url'] ?: 'images/default-story.svg'); ?>" alt="">
                                        <div>
                                            <strong><?php echo htmlspecialchars($s['release_title'] ?: ($s['title'] ?? 'Untitled')); ?></strong>
...
<script src="js/series_media_watch.js"></script>
