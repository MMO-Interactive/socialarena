(function attachV2ProjectEndpoints(globalScope) {
  function createProjectEndpoints(apiClient) {
    ...
    return {
      ...
      getTimeline: (token, studioId, type, id) => apiClient.request(withStudio(`editor/projects/${type}/${id}/timeline`, studioId), { token }),
      saveTimeline: (token, studioId, type, id, body) => apiClient.request(withStudio(`editor/projects/${type}/${id}/timeline`, studioId), { method: "POST", token, body }),
